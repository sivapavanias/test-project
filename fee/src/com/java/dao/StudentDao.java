...
public static int save(StudentBean bean){
	...
	try{
		...
		PreparedStatement ps=con.prepareStatement("insert into fee_student(Rollno,name,email,sex,course,fee,paid,due,address,contact) values(?,?,?,?,?,?,?,?,?,?)");
		//int newId = bean.getRollno();
...
public static int update(StudentBean bean){
	...
	try{
        ...
        //Long Rollno = request.getParameter("Rollno");
		ps.setString(2,bean.getName());
		ps.setString(3,bean.getEmail());
		ps.setString(4,bean.getSex());
		ps.setString(5,bean.getCourse());
